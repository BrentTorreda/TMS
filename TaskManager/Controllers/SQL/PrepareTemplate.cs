...
namespace TaskManager.Controllers.SQL
{
    public class PrepareTemplate
    {
        ...
        public int InsertTemplateForCompany(int id, int companyId)
        {
            var oldSubTasks = _context.SubTasksLevel1.Where(s => s.TaskId == id).ToList();
            int[] oldSubTaskIds = new int[20];
            int i = 0;
            ...
            //task
            SqlParameter param1 = new SqlParameter("param1", id);
            SqlParameter param2 = new SqlParameter("param2", companyId);
            int newTaskId = _context.Database.SqlQuery<Int32>("sp_addtasktemplate_forcompany @param1, @param2", param1, param2).FirstOrDefault();
